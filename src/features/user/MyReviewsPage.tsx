...
export default function MyReviewsPage() {
  ...
  const fetchMyReviews = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const response = await userService.getMyReviews(1, 100)
  ...
  useEffect(() => {
    ...
    fetchMyReviews()
  }, [isLoggedIn, navigate, fetchMyReviews])
            ...
            {reviews.map((review) => (
              <div key={review.id} className="rounded-xl bg-white p-4 shadow-sm">
                <div className="mb-2 flex items-center justify-between">
                  <h3 className="font-medium text-gray-900">{review.merchantName || '未知商家'}</h3>
                ...
                <div className="flex items-center justify-between text-xs text-gray-400">
                  ...
                  <button
                    ...
                    onClick={() => navigate(`/merchant/${review.merchantId}`)}
                  >
                    查看商家
                  </button>
                </div>
